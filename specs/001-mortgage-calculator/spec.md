...
4. **Given** 使用者選擇「貸款金額」模式且已輸入房屋總價 1000 萬，**When** 輸入貸款金額 800 萬，**Then** 系統自動計算並顯示貸款成數為 80.000%
5. **Given** 使用者選擇「貸款金額」模式且已輸入房屋總價 1000 萬，**When** 輸入貸款金額 825 萬，**Then** 系統自動計算並顯示貸款成數為 82.500%
6. **Given** 使用者已輸入房屋總價與貸款金額，**When** 修改房屋總價，**Then** 系統即時重新計算貸款成數
7. **Given** 使用者正在查看自動計算的成數，**When** 成數計算結果為非整數（如 82.5%），**Then** 系統以小數點後三位精度顯示（如 82.500%）
8. **Given** 使用者在「貸款金額」模式，**When** 未輸入房屋總價就輸入金額，**Then** 系統不顯示貸款成數（因無法計算）
...
2. **Given** 使用者選擇「浮動利率」，**When** 進入浮動利率設定，**Then** 系統顯示多階段利率輸入介面，預設顯示第一階段（階段、利率、起始年~結束年）
3. **Given** 使用者在浮動利率的多階段輸入介面，**When** 點擊「+」按鈕，**Then** 系統新增下一個階段的輸入列，階段編號自動遞增
4. **Given** 使用者已新增多個利率階段，**When** 點擊某階段的「刪除」按鈕，**Then** 該階段被移除，剩餘階段編號自動重新排序
5. **Given** 使用者輸入第一階段利率 1.5%（第 1~2 年）、第二階段 2.0%（第 3~5 年）、第三階段 2.5%（第 6~30 年），**When** 進行試算，**Then** 系統依據各階段利率分別計算該期間的還款金額
6. **Given** 使用者設定的階段年期總和不等於貸款年限，**When** 進行試算，**Then** 系統提示「階段年期總和需等於貸款年限」
7. **Given** 使用者設定的階段年期有重疊或間隙，**When** 進行試算，**Then** 系統提示年期設定有誤
8. **Given** 使用者在浮動利率模式，**When** 查看試算結果，**Then** 系統顯示各階段的還款明細與利率變動風險提示
...
**貸款金額與成數雙向計算相關**
- 當貸款金額超過房屋總價（成數 > 100%）時，系統應顯示警示並提示確認
- 當計算出的成數為無限小數（如 33.333...%）時，系統應四捨五入至小數點後三位（33.333%）
- 當房屋總價為 0 或未輸入時，系統不應計算或顯示成數
- 當使用者快速連續修改貸款金額時，成數計算應進行節流處理以維持效能
- 當成數計算結果為整數（如 80%）時，仍應顯示三位小數（80.000%）

**多階段浮動利率相關**
- 當使用者刪除中間階段時，系統應自動重新排序階段編號並重新計算年期
- 當使用者新增階段但剩餘年數為 0 時，系統應提示「已無可用年數分配」
- 當使用者輸入的階段利率為 0% 時，系統應正常計算並顯示該階段免息
- 當使用者輸入的階段利率超過合理範圍（如 > 20%）時，系統應顯示警示但仍允許計算
- 當使用者修改貸款年限後，最後一階段的結束年應自動調整以符合新年限
- 當使用者在多階段模式下切回固定利率時，系統應清除所有階段設定
- 當某階段的年期為 0（起始年等於結束年）時，系統應提示該階段無效
- 當使用者快速連續新增/刪除階段時，系統應維持介面穩定性
...
- **FR-007a**: 當選擇「浮動利率」時，系統必須顯示多階段利率輸入介面
- **FR-007b**: 多階段利率輸入介面必須包含：階段編號、該階段利率、起始年、結束年
- **FR-007c**: 系統必須提供「新增階段」按鈕（+），讓使用者可新增更多利率階段
- **FR-007d**: 系統必須提供「刪除階段」按鈕，讓使用者可移除已新增的階段（至少保留一個階段）
- **FR-007e**: 新增階段時，系統必須自動遞增階段編號並自動設定起始年為前一階段結束年的下一年
...
- **FR-011a**: 當使用「貸款金額」模式且已輸入房屋總價時，系統必須自動計算並顯示對應的貸款成數
- **FR-011b**: 貸款成數的計算公式為：成數 = (貸款金額 / 房屋總價) × 100%
- **FR-011c**: 貸款成數必須以小數點後三位精度顯示（例如：82.500%）
- **FR-011d**: 當房屋總價或貸款金額變更時，系統必須即時重新計算並更新成數顯示
- **FR-012**: 當選擇「浮動利率」時，系統必須依據各階段設定的利率與年期，分段計算各期間的還款金額
- **FR-012a**: 浮動利率計算必須驗證所有階段的年期總和等於貸款年限
- **FR-012b**: 浮動利率計算必須驗證各階段年期無重疊且無間隙（連續覆蓋整個貸款期間）

**輸出相關**

- **FR-013**: 系統必須顯示計算後的每月還款金額
- **FR-014**: 當選擇「等額本金」時，系統必須顯示每月還款遞減的說明資訊
- **FR-015**: 當選擇「浮動利率」時，系統必須顯示各階段還款明細（每階段的利率、期間、月付金額）及利率變動風險提示說明
...
- **利率設定（Interest Rate Settings）**: 包含利率類型、基準利率、浮動利率的多階段設定（各階段利率與年期範圍）等利率相關參數
- **多階段利率設定（Multi-Stage Rate Settings）**: 包含階段編號、該階段利率（%）、起始年、結束年，用於浮動利率的分段計算
...
- 固定利率：整個貸款期間利率維持不變
- 浮動利率：採用多階段利率設定，使用者可自定義各階段的利率與適用年期
- 多階段利率的年期總和必須等於貸款年限，且各階段年期需連續無間隙
- 多階段利率計算時，每階段使用該階段的固定利率進行試算
- 浮動利率的多階段設定僅為使用者的預期規劃，實際利率依各銀行公告為準
- 新增利率階段時，系統預設將起始年設為前一階段結束年的下一年
- 至少需保留一個利率階段，不可刪除所有階段
- 千分位格式使用逗號（,）作為分隔符號，符合台灣通用習慣
- 金額顯示預設取到整數位，不顯示小數點
- 貸款成數計算公式：成數 = (貸款金額 / 房屋總價) × 100%
- 貸款成數顯示精度為小數點後三位（例如：82.500%），四捨五入處理
- 貸款金額與成數為雙向同步關係：修改金額會自動計算成數，修改成數會自動計算金額
...
- **SC-026**: 貸款成數在貸款金額變更後 100ms 內即時更新顯示
- **SC-027**: 貸款成數顯示精確至小數點後三位（例如：82.500%），無精度遺失
- **SC-028**: 使用者能在 3 秒內透過貸款金額或貸款成數任一方式完成輸入切換
- **SC-029**: 使用者能在 10 秒內完成多階段浮動利率的設定（新增/編輯/刪除階段）
- **SC-030**: 新增利率階段後，階段編號與起始年在 100ms 內自動更新
- **SC-031**: 浮動利率的多階段計算結果在 200ms 內顯示各階段還款明細
- **SC-032**: 90% 的使用者能正確理解多階段浮動利率的年期設定方式
- **SC-033**: 階段年期驗證錯誤提示在 100ms 內顯示，使用者能在 5 秒內理解並修正
